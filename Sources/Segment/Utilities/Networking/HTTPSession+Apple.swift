...
extension URLSession: HTTPSession {
  public func uploadTask(with request: URLRequest, fromFile file: URL, completionHandler: @escaping (Data?, URLResponse?,  Error?) -> Void) -> any DataTask {
      let task: URLSessionUploadTask = uploadTask(with: request, fromFile: file, completionHandler: completionHandler)

      return task
  }

  public func dataTask(with request: URLRequest, completionHandler: @escaping (Data?, URLResponse?, Error?) -> Void) -> any DataTask {
